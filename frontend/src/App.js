...
function App() {
  ...
  const handleSwitchSession = async (sessionId) => {
    try {
      await axios.post('/api/sessions/switch', { targetSessionId: sessionId });
      setSessionDialogOpen(false);
      loadTasks();
      loadSessions();
    } catch (err) {
      alert('切换会话失败: ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const handlePause = async (taskId) => {
    try {
      await axios.post(`/api/pause/${taskId}`);
      loadTasks(); // 刷新任务状态
    } catch (err) {
  ...
  const handleResume = async (taskId) => {
    try {
      await axios.post(`/api/resume/${taskId}`);
      loadTasks(); // 刷新任务状态
    } catch (err) {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
        ...
        >
          {currentSessionId ? `会话: ${currentSessionId.substring(0, 8)}` : '会话列表'}
        </Button>
          ...
          <List>
            {sessions.map((session) => (
              <ListItem key={session.id} disablePadding>
                ...
                >
                  <ListItemText
                    primary={`会话 ${session.id.substring(0, 8)}...`}
                    secondary={`创建于: ${new Date(session.createdAt).toLocaleString()} - ${session.taskCount || 0} 个任务`}
                  />
                  {session.id === currentSessionId && <Chip label="当前" size="small" color="primary" />}
                </ListItemButton>
              </ListItem>
            ))}
            {sessions.length === 0 && (
              <Typography align="center" sx={{ py: 2 }} color="text.secondary">无可用会话</Typography>
            )}
          </List>
        </DialogContent>
      </Dialog>
