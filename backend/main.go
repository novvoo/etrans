...
func main() {
	...
	api := r.Group("/api")
	{
		...
		api.POST("/pause/:taskId", handlers.PauseTaskHandler)
		api.POST("/resume/:taskId", handlers.ResumeTaskHandler)
		api.GET("/sessions", handlers.GetSessionsHandler)
		api.POST("/sessions/switch", handlers.SwitchSessionHandler)
	}
